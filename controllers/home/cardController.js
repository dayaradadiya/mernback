...
class cardController {
     ...
     get_card_products = async(req,res) => {
        ...
        for (let i = 0; i < stockProduct.length; i++) {
            ...
            if (discount !== 0) {
                calculatePrice = calculatePrice + quantity * (price -  ((price * discount) / 100))
            } else {
        ...
        for (let j = 0; j < stockProduct.length; j++) {
           ...
           if (unique[i] === tempProduct.sellerId.toString()) {
                ...
                if (tempProduct.discount !== 0) {
                    pri = tempProduct.price - ((tempProduct.price * tempProduct.discount) / 100 )
                    test_pri = tempProduct.price - ((tempProduct.price * tempProduct.discount) / 100 )
                } else {
                    pri = tempProduct.price
                    test_pri = tempProduct.price
                }
        pri = pri - ((pri * co) / 100)
